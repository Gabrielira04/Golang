...
func ApagarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	statement, erro := db.Prepare("delete from usuarios where CPF = ?")
	if erro != nil {
		fmt.Println("erro")
		fmt.Printf("%#v \n", erro)
...
func LocalizarUsuarios(w http.ResponseWriter, r *http.Request) {
	...
	for linhas.Next() {
		...
		if erro := linhas.Scan(&usuario.Name, &usuario.Cpf, &usuario.PhoneNumber, &usuario.CelNumber, &usuario.FaxNumber, &usuario.Email, &usuario.Cep); erro != nil {
			fmt.Printf("%#v \n", erro)
...
func LocalizarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	var usuario usuarios
	if linha.Next() {
		if erro := linha.Scan(usuarios.Name, usuarios.Cpf, usuarios.PhoneNumber, usuarios.CelNumber, usuarios.FaxNumber, usuarios.Email, usuarios.Cep); erro != nil {
